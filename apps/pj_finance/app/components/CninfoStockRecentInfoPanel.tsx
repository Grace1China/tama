...
export type CninfoStockRecentInfoPanelProps = {
  tsCode: string;
  /** 父级弹框打开时为 true，用于触发拉数 */
  active?: boolean;
};

/** 巨潮 p_info3085 列表 + PDF 预览（无 portal/标题栏） */
export function CninfoStockRecentInfoPanel({ tsCode, active = true }: CninfoStockRecentInfoPanelProps) {
  ...
  useEffect(() => {
    if (active && scode) void load();
  }, [active, scode, load, tsCode]);
  ...
  const subtitle = scode ? `${tsCode}（巨潮 p_info3085 · ${scode}）` : tsCode;
          ...
          <div className="shrink-0 border-b border-gray-100 bg-gray-50 px-3 py-2 text-xs text-gray-600">
            {loading
              ...
              : err
                ? `加载失败：${err}`
                : payload
                  ? `共 ${payload.total ?? sortedRecords.length} 条，本页展示 ${sortedRecords.length} 条（按发布时间倒序）· ${subtitle}`
                  : '无数据'}
          </div>
