  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (sortField && sortDir && /^[a-zA-Z0-9_]+$/.test(sortField)) {
        ...
        orderByClause = sortField === 'end_date'
          ? `ORDER BY ${endDateExpr} ${dir}`
      ...
      conn.all(countSql, (countErr: Error | null, countRows: any[]) => {
        ...
        conn.all(dataSql, (err: Error | null, rows: any[]) => {
          ...
          const chineseHeaders = mapHeadersToChinese(originalHeaders, 'finaMainbzVip');
...
export async function GET(request: NextRequest) {
  try {
    ...
    const parquetPath = path.join(process.cwd(), 'temp/tuShare/fina_mainbz_vip_ss.parquet');
